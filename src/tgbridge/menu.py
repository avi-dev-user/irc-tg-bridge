...
def perform_menu(t, server: str, count: int) -> Menu:
    ...
    callback_data), an Add action, and a back button to the server settings."""
    rows: Menu = []
    for i in range(count):
        rows.append([(f"{t('perform.remove')} #{i + 1}",
                      cb("srv", "permdel", f"{server}.{i}"))])
    rows.append([(t("perform.add"), cb("srv", "permadd", server))])
    rows.append([(t("menu.back"), cb("srv", "settings", server))])
    return rows
